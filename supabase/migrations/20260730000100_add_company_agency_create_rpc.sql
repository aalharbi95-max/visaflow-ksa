...
-- Browser roles retain read-only table access. All agency creation is mediated
-- by the tenant-bound function below.
revoke insert, update, delete on table public.agencies
  from public, anon, authenticated;
revoke insert, update, delete on table public.company_agency_access
  from public, anon, authenticated;
...
begin
  ...
  agency_contact_person :=
    pg_catalog.btrim(coalesce(p_payload->>'contact_person', ''));
  agency_email := nullif(
  ...
  if agency_email is not null then
    perform pg_catalog.pg_advisory_xact_lock(
      pg_catalog.hashtextextended(
  ...
  where (
      ...
      and pg_catalog.lower(pg_catalog.btrim(coalesce(agency.email, ''))) =
        agency_email
    )
    or (
      pg_catalog.lower(pg_catalog.btrim(coalesce(agency.name, ''))) =
        pg_catalog.lower(agency_name)
      and pg_catalog.lower(pg_catalog.btrim(coalesce(agency.country, ''))) =
        pg_catalog.lower(agency_country)
    )
  ...
  if agency_row.id is not null then
    ...
    raise exception 'COMPANY_AGENCY_CREATE_ALREADY_EXISTS';
  end if;
