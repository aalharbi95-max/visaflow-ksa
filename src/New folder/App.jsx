...
const [currentUser, setCurrentUser] = useState(() => {
  try {
    return JSON.parse(localStorage.getItem("visaflow_user") || "null");
  } catch {
    return null;
  }
});
...
arrival_date: item.arrival_date || "",
  ...
  .from("requests")
  .select("approval_status, quantity")
...
medical_status: candidateForm.arrival_date || candidateForm.flight_date || candidateForm.ticket_no
  ? "Passed"
  : candidateForm.medical_status || "Pending",
medical_date: candidateForm.medical_date || null,
ticket_no: candidateForm.ticket_no || "",
flight_date: candidateForm.flight_date || null,
arrival_date: candidateForm.arrival_date || null,
...
const { count: arrivedCount } = await supabase
  ...
  .eq("request_no", candidateForm.request_no)
  .eq("status", "Arrived KSA");

const requestRemaining =
  Number(requestData?.quantity || 0) - Number(arrivedCount || 0);
...
setActivePage("Candidates");
 ...
 async function saveInterview() {
  ...
  const result = interviewEditingId
    ...
    : await supabase.from("interviews").insert([interviewPayload]);
...
async function handleLogin() {
  ...
  const { data, error } = await supabase
    .from("users")
    .select("id, name, email, role, agency_name, status")
  ...
  localStorage.setItem("visaflow_user", JSON.stringify(data));
  setCurrentUser(data);
  setActivePage((ROLE_PAGES[data.role] || ROLE_PAGES.Viewer)[0]);
}
  ...
  return (
    <div className="login-page">
      <div className="login-card">
        <div className="login-logo">VF</div>
        <h1>VisaFlow KSA</h1>
        <p>Recruitment & Visa Operations Login</p>
        ...
        <button className="save-btn login-btn" onClick={handleLogin} disabled={loginLoading}>
      ...
      <main className="content">
        <div className="topbar sticky">
          ...
          <div className="actions-line">
            ...
            <button className="new-btn" onClick={exportCurrentPage}>Export Excel</button>
...
<button onClick={() => editCandidate(item)}>Edit</button>
