...
async function resolveLocalImport(importer, specifier) {
  ...
  candidates.push(
    ...SOURCE_EXTENSIONS.slice(1).map((extension) => path.join(base, `index${extension}`))
  );
  const resolved = await firstExistingFile(candidates);
  if (!resolved) {
    throw new Error(`Unresolved local production import "${specifier}" from ${importer}`);
  }
...
export async function findProtectedTableMutationViolations(graph) {
  const violations = [];
  const sourceFiles = graph.files.filter((file) => /\.(?:js|jsx|mjs|ts|tsx)$/.test(file));
...
export async function runProductionMutationGuard({
  ...
  if (violations.length) {
    const details = violations
      .map((item) => `${item.file}:${item.line} ${item.table}.${item.operation}`)
      .join("\n");
...
if (invokedAsScript) {
  ...
  console.log(`Protected-table mutation guard passed for ${graph.files.length} production files.`);
  for (const file of graph.files) console.log(path.relative(workspace, file));
}
