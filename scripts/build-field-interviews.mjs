...
import {sectors,roles,levels} from './field-interview-data.mjs';
export const release = 'VF-FIELD-20260911';
...
function makeQuestions(r,l) {
  ...
  qs[3]=['Technical','Practical verification',10,r.practical.ar+' '+depthAr,r.practical.en+' '+depthEn,[...r.practical.points,...extra],r.practical.ideal];
...
assert.equal(templates.length,75);
assert.equal(new Set(templates.map(t=>t.name)).size,75);
...
fs.writeFileSync('src/fieldInterviewCatalog.mjs',`// Public catalog metadata only. Questions and scoring references remain in the database.\nexport const FIELD_INTERVIEW_CATALOG = ${JSON.stringify(catalog,null,2)};\n`);
fs.writeFileSync('src/fieldTalentCampaigns.mjs',`export const FIELD_TALENT_CAMPAIGNS = ${JSON.stringify(sectors.map(s=>({slug:s.slug,nameAr:`حملة ${s.ar}`,nameEn:`${s.en} Campaign`,descriptionAr:`${15+s.reuseIds.length} قالب مقابلة في ${s.ar}: مبتدئ، ممارس ومشرف فريق.`,descriptionEn:`${15+s.reuseIds.length} interview templates in ${s.en}: entry level, professional and team supervisor.`,professionLabelAr:'اختر المسار والمستوى الوظيفي',professionLabelEn:'Select your role and level',badgeAr:`VisaFlow — ${s.ar}`,badgeEn:`VisaFlow — ${s.en}`,headlineAr:`اختبر جاهزيتك في ${s.ar}.`,headlineEn:`Test your readiness in ${s.en}.`,introAr:'اختر المسار والمستوى المناسبين. القوالب الجديدة تتضمن ٨ أسئلة بالعربية أو الإنجليزية، وتحتفظ القوالب السابقة بأسئلتها الأصلية. تقبل أمثلة الدراسة والتدريب للمبتدئين. مشاركة السيرة وبيانات التواصل مع الشركات مطلوبة؛ مشاركة النتيجة اختيارية. المقابلة لا تضمن التوظيف ولا تمنح ترخيصًا مهنيًا.',introEn:'Choose the appropriate role and level. New templates have 8 questions in Arabic or English; existing templates retain their original questions. Study and training examples are welcome at entry level. CV and contact sharing with employers are required; result sharing is optional. This interview does not guarantee employment or confer a professional license.'})),null,2)};\n`);
const sql=`-- 75 approved bilingual templates for 25 missing field roles; existing technicians are reused.
do $seed$
declare
...
begin
  ...
  if (select count(*) from public.ai_interview_templates where company_id=v_company_id and is_global and is_active and is_current_version and status='Active' and approval_status='Approved' and id::text in (select jsonb_array_elements_text($reuse_ids$${JSON.stringify(sectors[0].reuseIds)}$reuse_ids$::jsonb))) <> 8 then raise exception 'Reviewed reusable templates must remain active and approved'; end if;
  ...
  for c in select value from jsonb_array_elements($campaigns$${JSON.stringify(sectors)}$campaigns$::jsonb) loop
    ...
    values (c->>'slug','VisaFlow '||(c->>'en')||' Campaign','حملة VisaFlow — '||(c->>'ar'),
      ...
      v_company_id,'Active',now(),jsonb_build_object('template_filter',c->>'filter','catalog_release','${release}','nationality_restriction',false,'existing_template_ids',coalesce(c->'reuseIds','[]'::jsonb)))
    on conflict (slug) do nothing;
  end loop;
end;
$seed$;
`;
const previous=fs.readFileSync('supabase/migrations/20260911000200_expand_interview_library.sql','utf8');
const eligibility=previous.slice(previous.indexOf('create or replace function public.talent_campaign_template_is_eligible'))
  .replace("when 'junior_accounting'",sectors.map(s=>`when '${s.filter}' then lower(coalesce(template.profession_category, '')) = '${s.en.toLowerCase()}' and template.ai_analysis->>'catalog_release' = '${release}' or coalesce(campaign.settings->'existing_template_ids','[]'::jsonb) ? template.id::text`).join('\n        ')+"\n        when 'junior_accounting'");
fs.writeFileSync('supabase/migrations/20260911000300_field_interview_library.sql',sql+'\n'+eligibility);
