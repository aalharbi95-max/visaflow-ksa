...
const expectedMigrationSha256 = "01fb4d0f5dc33e72165aa8bec16715c1425301606934ce366c0c1fdb546e8fc1";
...
async function precheck() {
  ...
  const timeoutStartMarker = "set local statement_timeout = '180s';";
  const blockMarker = "do $precheck$";
  const blockEndMarker = "$precheck$;";
  const timeoutResetMarker = "set local statement_timeout = '60s';";
  const start = migration.indexOf(timeoutStartMarker);
  const blockStart = migration.indexOf(blockMarker, start);
  const blockEnd = migration.indexOf(blockEndMarker, blockStart);
  const reset = migration.indexOf(timeoutResetMarker, blockEnd + blockEndMarker.length);
  assert.ok(start >= 0 && blockStart > start && blockEnd > blockStart && reset > blockEnd,
    "Exact bounded migration precheck block not found");
  const precheckSql = migration.slice(start, reset + timeoutResetMarker.length);
